...
passport.use(new GoogleStrategy({
    ...
    if (existingUser) {
        ...
        const user = await new User({oauthID: profile.id, username: profile.displayName, oauthProvider: profile.provider, email: profile.email, photo: profile.photos[0].value}).save();
        done(null, user);
    }
}));
