...
router.delete('/restaurant/:restaurantId/unsaved', function (req, res) {
    Restaurant
        .findById(req.params.restaurantId, function (err, dislikedRestaurant) {
            if (err) {
                console.log(err);
                res.status(400);
                res.json({"error": "restaurant not found"});
                ...
                var removeIndexForRestaurant = dislikedRestaurant
                    .likedUser
                    .map(function (user) {
                        return user._id;
                    })
                ...
                if (removeIndexForRestaurant === -1) {
                    ...
                    res.send({
                        message: "User id " + req.user._id + " dislike restaurant id " + dislikedRestaurant._id
                    });
                }
            }
        });
        ...
        User
            .findById(req.params.userId, function (err, user) {
                if (user) {
                    var visited = 0;
                    var restaurantMap = [];
                    for (var i = 0; i < user.likedRestaurants.length; i++) {
                        (function (i) {
                            Restaurant
